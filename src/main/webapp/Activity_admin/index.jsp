...
function del(id) {
	layer.confirm('确认删除吗？删除后不能恢复，谨慎操作',function(){
		$.ajax({
	          //几个参数需要注意一下
	              ...
	              url: "../Activity_admin/delete_json?id="+id,//url
	...
	<div class="block-area" id="tableStriped">
		...
		<form action="../Activity_admin/index" method="post">	
			<button class="btn btn-sm btn-alt m-r-5" type="button"
				onclick="openwin('../Activity_admin/add','500','300')">新增</button>
					...
					<c:forEach items="${requestScope.list}" var="r">
						<tr>
							...
							<td><a class="glyphicon glyphicon-pencil"
								href="javascript:;"
								onclick="openwin('../Activity_admin/edit?id=${r.id}','500','300')">修改</a>
		...
		layui.use('laypage', function(){
			  ...
			  laypage.render({ 
				    ...
				    ,jump: function(obj,first){
				      if(!first){
				    	  location.href="../Activity_admin/index?pageno="+obj.curr;
				        } 
				    }
				  });
			});
	...
	//		$($(".sinput")[select]).css("display","inline").removeAttr("disabled").val(txt);
		...
		$(function() {
			if(${fn:length(requestScope.list)==0&&requestScope.count!=0}){ 
				 ...
				 location.href="../Activity_admin/index?pageno="+pageno;
			}
		});
